...
DROP PROCEDURE IF EXISTS IsAvailableUserName;
DROP PROCEDURE IF EXISTS MatchPassword;
...
BEGIN
    ...
    VALUES 
        (_FIRST_NAME, _LAST_NAME, _DEPT_ID, _USER_NAME, SHA(CONCAT(_LAST_NAME, '1234')), _USER_GROUP);
...
BEGIN
	...
	ON
		S.ROOM_ID = R.ROOM_ID
	ORDER BY
		SCHEDULE_DATE DESC, SCHEDULE_TIME DESC, P.PATIENT_NAME ASC;
END $$
DELIMITER ;
...
BEGIN
	SET SQL_MODE = ''; 
	INSERT INTO
		Schedules (PATIENT_ID, DOC_ID, SCHEDULE_DATE, SCHEDULE_TIME, LOC_ID, ROOM_ID, SCHEDULE_NOTES)
	VALUES
		(_PATIENT_ID, _DOC_ID, STR_TO_DATE(_SCHEDULE_DATE, '%Y-%m-%d'), STR_TO_DATE(_SCHEDULE_TIME, '%H:%i'), _LOC_ID, _ROOM_ID, _SCHEDULE_NOTES);
...
DELIMITER $$
CREATE PROCEDURE SetUserLogin(
	IN _USER_NAME VARCHAR(40),
    IN _USERPASSWORD VARCHAR(255)
)
BEGIN
	SELECT
		U.USER_ID, U.FIRST_NAME, U.LAST_NAME, D.DEPT_ID, DEPT_NAME,
        U.USER_NAME, U.USER_GROUP AS USER_GROUP_ID, UG.GROUP_NAME
    FROM
		Users U
	INNER JOIN
		Departments D
	...
	INNER JOIN
		UserGroups UG
	ON
		U.USER_GROUP = UG.GROUP_ID
	WHERE
		U.USER_NAME = _USER_NAME AND U.USER_PASSWORD = SHA(_USERPASSWORD);
END $$
DELIMITER ;
	
    
DELIMITER $$
CREATE PROCEDURE IsAvailableUserName(IN _USER_NAME VARCHAR(40))
BEGIN
	SELECT
		CASE WHEN COUNT(USER_ID) > 0 THEN 'Unavailable' ELSE 'Available' END IS_AVAILABLE
	FROM
		Users
	WHERE
		USER_NAME = _USER_NAME;
...
DELIMITER $$
CREATE PROCEDURE MatchPassword(
	IN _USER_ID VARCHAR(40), 
    IN _USER_PASSWORD VARCHAR(255)
)
BEGIN
	SELECT
		CASE WHEN COUNT(USER_ID) > 0 THEN 'Unmatched' ELSE 'Matched' END IS_MATCH
	FROM
		Users
	WHERE
		USER_NAME = _USER_NAME AND USER_PASSWORD = _USER_PASSWORD;
END $$
DELIMITER ;